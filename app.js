(() => {
  ...
  let real={routeKey:null,index:0,speed:0,gear:'P',signal:'—',wheel:0,brakeHeld:false,accelHeld:false,distanceToNext:0,segmentLength:1,finished:false};
  ...
  // Google Maps Embed API. V4 uses three Street View panes in landscape and
  // continuous on-screen driving controls. Street View still advances between
  // preselected road checkpoints because Maps Embed does not expose vehicle physics.
  function initGoogleStatus(){
    if(googleKey){els.googleStatus.textContent='Google 준비됨';els.googleStatus.classList.add('ready');els.googleSetup.classList.add('hidden')}
  ...
  function streetViewUrl(point,offset=0){
    const p=new URLSearchParams({key:googleKey,location:`${point.lat},${point.lng}`,heading:String(normalizeHeading((point.heading||0)+offset)),pitch:'0',fov:offset===0?'78':'72'});
  ...
  function resetSegmentDistance(){
    const route=routes[real.routeKey];
    if(!route||real.index>=route.points.length-1){real.segmentLength=1;real.distanceToNext=0;return}
    real.segmentLength=Math.max(15,Math.round(haversine(route.points[real.index],route.points[real.index+1])));
    real.distanceToNext=real.segmentLength;
  }
  function startRealRoute(key){
    if(!googleKey){els.googleSetup.classList.remove('hidden');els.googleSetup.scrollIntoView({behavior:'smooth',block:'center'});return}
    real={routeKey:key,index:0,speed:0,gear:'P',signal:'—',wheel:0,brakeHeld:false,accelHeld:false,distanceToNext:0,segmentLength:1,finished:false};
    ...
    els.realRouteTitle.textContent=routes[key].title;
  ...
  function renderRealPoint(){
    const route=routes[real.routeKey],point=route.points[real.index];
    els.streetViewFrame.src=streetViewUrl(point,0);
    if(els.streetViewLeft)els.streetViewLeft.src=streetViewUrl(point,-52);
    if(els.streetViewRight)els.streetViewRight.src=streetViewUrl(point,52);
    els.satelliteFrame.src=satelliteUrl(point);
    els.realLevelLabel.textContent=`실도로 ${real.index+1}/${route.points.length}`;
    els.realPointName.textContent=point.name;
    els.realMissionBubble.innerHTML=`<span>현재 훈련</span><strong>${point.mission}</strong>`;
    els.checkpointLabel.textContent=`${real.index+1} / ${route.points.length}`;
    els.realPrevBtn.disabled=real.index===0;
    els.realNextBtn.textContent=real.index===route.points.length-1?'처음으로':'다음 지점';
    updateRealHud();
  }
  function updateRealHud(){
    ...
    if(els.realDistance){
      if(real.index>=routes[real.routeKey]?.points.length-1)els.realDistance.textContent='도착';
      else els.realDistance.textContent=`${Math.max(0,Math.round(real.distanceToNext))} m`;
    }
    if(els.realProgressBar){
      const p=real.index>=routes[real.routeKey]?.points.length-1?100:Math.max(0,Math.min(100,(1-real.distanceToNext/Math.max(1,real.segmentLength))*100));
      els.realProgressBar.style.width=`${p}%`;
    }
  }
  function setRealFeedback(msg,type='neutral'){els.realFeedback.textContent=msg;els.realFeedback.className=`feedback ${type}`}
  function advanceReal(delta=1,{manual=false}={}){
    const route=routes[real.routeKey];if(!route)return;
    if(delta>0&&real.index>=route.points.length-1){
      if(manual){real.index=0;real.finished=false;real.speed=0;resetSegmentDistance();renderRealPoint()}
      return;
    }
    let next=Math.max(0,Math.min(route.points.length-1,real.index+delta));
    real.index=next;real.finished=real.index===route.points.length-1;resetSegmentDistance();renderRealPoint();
    if(real.finished){real.speed=0;setRealFeedback('코스 마지막 지점에 도착했습니다. 완전히 정차한 뒤 P로 변경해보세요.','good')}
  }
  function driveLoop(ts){
    ...
    if(real.routeKey){
      ...
      else if(real.accelHeld&&real.gear==='D'&&!real.finished)real.speed=Math.min(50,real.speed+11*dt);
      else real.speed=Math.max(0,real.speed-1.8*dt);

      if(real.gear!=='D'&&real.speed>0)real.speed=Math.max(0,real.speed-8*dt);
      if(real.gear==='D'&&real.speed>.3&&!real.finished){
        real.distanceToNext-=real.speed/3.6*dt;
        if(real.distanceToNext<=0)advanceReal(1);
      }
      updateRealHud();
    }
  ...
  function startPedal(action,e){
    ...
    setRealFeedback(action==='brake'?'브레이크를 밟고 있습니다. 속도가 연속적으로 줄어듭니다.':'가속 페달을 밟고 있습니다. 시선은 멀리 두세요.','good');
  }
  ...
  function handleRealAction(action){
    ...
    if(action==='gearD'){
      ...
      real.gear='D';setRealFeedback('D 선택. 브레이크에서 발을 천천히 떼고 주변을 확인하세요.','good');
      ...
      real.gear='R';real.speed=0;setRealFeedback('R 선택. 거울과 직접 확인을 번갈아 하세요.','good');
      ...
      real.signal='◀';setRealFeedback('좌측 방향지시등. 미러 → 사각지대 → 조향 순서로 확인하세요.','good');
    }else if(action==='rightSignal'){
      real.signal='▶';setRealFeedback('우측 방향지시등. 보행자와 이륜차까지 확인하세요.','good');
  ...
  $('#closeRealBtn').onclick=()=>{
    real.brakeHeld=false;real.accelHeld=false;real.speed=0;real.routeKey=null;
    els.realTraining.classList.add('hidden');document.body.classList.remove('simulator-mode','show-nav');
  };
  ...
  els.realPrevBtn.onclick=()=>advanceReal(-1,{manual:true});
  els.realNextBtn.onclick=()=>advanceReal(1,{manual:true});
